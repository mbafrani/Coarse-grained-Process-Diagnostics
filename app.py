...
app.secret_key = "diagnostic-tooooool"
...
def result():
    ...
    if request.method == 'POST':
        try:
            if "prev_event_log" in request.form.keys():
                event_log_address = session['ready_event_log_path']  # contains Outputs\\ready_event_log.csv
        ...
        if inactive != "on":
            if aspect == "General":
                generated_SD_log.append(
                    (com_sd.TW_discovery_process_calculation_twlist(time_window, event_log[0], aspect))[0])
            if aspect == 'Organizational':
                ...
                for org in list(org_list.keys()):
                    ...
                    if len(filtered_log['Case ID']) > 1:
                        generated_SD_log.append(
                            (com_sd.TW_discovery_process_calculation_twlist(time_window, filtered_log, org))[0])
            ...
            if aspect == 'Activity Flow':
                ...
                for act in act_list:
                    ...
                    generated_SD_log.append(
                        str(com_sd.select_features(features_list, event_log[0], time_window, "Activities")))
                """
                for act in act_list:
                    act_list_filter = []
            ...
            if aspect == 'Resources':
                ...
                for act in act_list:
                    ...
                    generated_SD_log.append(
                        (com_sd.select_features(features_list, event_log[0], time_window, "Resources")))
                """
                for act in act_list:
                    act_list_filter = []
                    act_list_filter.append(act)
                ...
                if acreslist[0] in res_list:
                    ...
                    filtered_log = org_asp.filter_log_res(event_log[0], act_list_filter)
                    if len(filtered_log['Case ID']) > 1:
                        generated_SD_log.append((com_sd.TW_discovery_process_calculation_twlist(time_window,
                                                                                                filtered_log,
                                                                                                request.form[
                                                                                                    "AcReList"]))[0])
                if acreslist[0] in act_list:
                    ...
                    if len(filtered_log['Case ID']) > 1:
                        generated_SD_log.append(
                            (com_sd.TW_discovery_process_calculation_twlist(time_window, filtered_log,
                                                                            request.form["AcReList"]))[0])
        else:
            if aspect == "General":
    ...
    session['SDLog'] = generated_SD_log
...
def get_SD_log():
    ...
    if request.method == 'POST':
        ...
        allcorr = rel_sd.show_corr_auto(os.path.join('Outputs', SD_log_address.filename), Corr_Lin, Corr_NLin,
                                        Corr_Lin_Thre, Corr_NLin_Thre, TWshift)
